...
## 4. Phase 2: Unbounded Hierarchical Nesting

While the Recurrent SSM excels at Markovian state-tracking (DFA), it fundamentally fails at context-free formal languages requiring an unbounded pushdown stack (e.g., the Dyck-$n$ language of deeply nested brackets).

To test the capacity thresholds of modern architectures, we introduced a **Nested Brackets Benchmark** where sequences are generated using a LIFO stack.

### 4.1. Depth-vs-Accuracy Profiling

We evaluated four architectures on their ability to predict the correct closing bracket at increasing nesting depths:

1. **Causal Self-Attention**: Fails to capture sequential pairing depth.
2. **Recurrent SSM**: Fails at deep nesting due to fixed-size continuous state bottleneck (memory capacity overflow).
3. **Stack-Augmented RNN**: A theoretical baseline augmented with a differentiable Pushdown Stack memory tape.
4. **Liquid State Machine (LSM)**: A sparse, randomly connected reservoir initialized strictly on the "Edge of Chaos" ($\lambda \approx 0$).

### 4.2. Phase 2 Key Findings

- **Architectural Failure**: Both the Transformer and the standard Recurrent SSM suffer catastrophic failure when sequence nesting exceeds their capacity thresholds.
- **Stack-RNN Success**: Only the Stack-Augmented RNN (whose architecture matches the formal language complexity of a Pushdown Automaton) achieves **100% accuracy at infinite nesting depth**.
- **Edge of Chaos Dynamics**: The Liquid State Machine demonstrates that chaotic reverberations can simulate a LIFO stack up to shallow depths, but ultimately decays when asked to perform deep, lossless memory retrieval compared to explicit stack structures.
