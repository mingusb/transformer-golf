...
def train_and_eval(model, train_len, test_len, epochs=300):
    vocab_size = 4
    num_samples = 100
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            ...
            with torch.no_grad():
                ...
                train_seq_acc = (train_preds == Y_train).all(dim=-1).float().mean().item()
                
                test_logits, _ = model(X_test)
                test_preds = test_logits.argmax(dim=-1)
                test_seq_acc = (test_preds == Y_test).all(dim=-1).float().mean().item()
                
                print(f"Epoch {epoch+1:03d} | Loss: {loss.item():.4f} | Train Seq Acc: {train_seq_acc:.4f} | Test Seq Acc: {test_seq_acc:.4f}")
                if train_seq_acc == 1.0 and test_seq_acc == 1.0:
                    break

print("=== STACK RNN ===")
stack_rnn = StackRNN(vocab_size=4, hidden_size=32, stack_width=4, stack_depth=20)
train_and_eval(stack_rnn, train_len=3, test_len=6, epochs=300)

print("\n=== DUAL STACK RNN ===")
dual_stack_rnn = DualStackRNN(vocab_size=4, hidden_size=32, stack_width=4, stack_depth=20)
train_and_eval(dual_stack_rnn, train_len=3, test_len=6, epochs=300)
